    ...
    for subject_index, subject in enumerate(subjects):
        ...
        for file in sub_files:
            ...
            run_output_df.loc[run_index] = [
                subject.name, file.split('/')[-1], run_FD_mean, run_pct_points_over]
            run_index += 1
        ...
        sub_output_df.loc[subject_index] = [subject.name, FD_mean,
                                            pct_points_over]

    run_output_df.to_csv(dir_tree.analysis_dir + 'FD_stats_runs.csv',
                         index=False)
