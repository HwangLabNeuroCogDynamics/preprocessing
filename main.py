...
def init_argparse() -> argparse.ArgumentParser:
    ...
    dirs_grp.add_argument('--bids_dir',
                          help='Path for bids directory if not located in '
                          'dataset directory.')
    dirs_grp.add_argument('--work_dir',
        ...
        if not args.no_default:
            for def_column in s.DEFAULT_COLUMNS:
                if def_column not in args.columns:
                    ...
                    print(f'You entered the column {def_column} that already '
...
def setup_mema(dir_tree, args, subjects, base_bashfile):
    ...
    return bashwriter.write_file(bashfile, base_bashfile.output_dir,
                                 base_bashfile.job_name)
